...
export default defineType({
  ...
  fields: [
    defineField({
      ...
      validation: (Rule) => Rule.required().error('Article type name is required'),
    }),
    defineField({
      name: 'slug',
      title: 'Slug',
    ...
    defineField({
      ...
      options: {
        list: [
          ...
          { title: 'Purple - Deep dives, advanced topics, research', value: 'purple' },
          { title: 'Red - Breaking news, alerts, important updates', value: 'red' },
          { title: 'Orange - Opinions, editorials, community content', value: 'orange' },
          { title: 'Yellow - Tips, quick wins, productivity', value: 'yellow' },
          { title: 'Pink - Reviews, comparisons, evaluations', value: 'pink' },
          { title: 'Indigo - Guides, how-tos, step-by-step content', value: 'indigo' },
          { title: 'Gray - Archive, legacy content, neutral topics', value: 'gray' },
        ],
        layout: 'radio',
      },
      initialValue: 'blue',
      validation: (Rule) => Rule.required().error('Color is required for badge display'),
    }),
  ],
  preview: {
    select: {
