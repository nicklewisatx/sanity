...
describe('CI Parity Validation', () => {
  ...
  describe('validateCIParity', () => {
    ...
    it('should error when pnpm-lock.yaml is missing', () => {
      ...
      expect(result.errors).toContain('pnpm-lock.yaml not found. Run "pnpm install" to generate it.')
    })
    ...
    it('should error when Node version is too old', () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors).toContain('Node.js version 20 or higher required. Current: v18.0.0')
    })
