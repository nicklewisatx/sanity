...
export function validateCIParity(projectRoot = process.cwd()): CIValidationResult {
  const errors: string[] = []
  ...
  if (process.env.CI !== 'true' && !process.argv.includes('--frozen-lockfile')) {
    warnings.push('Not using --frozen-lockfile. CI uses this flag to ensure reproducible builds.')
  }
...
export function assertCIParity(projectRoot?: string): void {
  ...
  if (!result.valid) {
    ...
    throw new Error('CI parity validation failed. Fix the errors above to match CI environment.')
  }
}