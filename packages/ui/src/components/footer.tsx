      ...
      >
        <div className="container mx-auto px-4">
          ...
          <div className="grid gap-8 lg:grid-cols-12">
            ...
            {(logo || description || socialLinks) && (
              <div className="lg:col-span-4">
                ...
                {description && (
                  <p className={cn("mb-4 text-sm", linkClass)}>{description}</p>
                )}
            ...
            {sections && sections.length > 0 && (
              ...
              >
                {sections.map((section, index) => (
                  <div key={index}>
                    <h3
                      className={cn("mb-4 text-sm font-semibold", headingClass)}
                    >
                      {section.title}
                    </h3>
          ...
          {(copyright || bottomLinks) && (
            ...
            >
              {copyright && (
                <p className={cn("text-sm", linkClass)}>{copyright}</p>
              )}
                ...
                <nav className="flex flex-wrap gap-4 sm:gap-6">
                  {bottomLinks.map((link, index) => (
                    <a
                      key={index}
                      href={link.href}
                      className={cn("text-sm transition-colors", linkClass)}
                    >
                      {link.label}
                    </a>
                  ))}
